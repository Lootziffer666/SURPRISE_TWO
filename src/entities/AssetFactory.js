import {
  BoxGeometry, CapsuleGeometry, Color, ConeGeometry, CylinderGeometry, Group,
  Mesh, MeshStandardMaterial, SphereGeometry, TorusGeometry, Vector3,
} from 'three';
...
const COLORS = Object.freeze({
  ...
  bear: '#f5f7f9', bearDark: '#dfe4ea', nose: '#3a3f46', wood: '#9f6442', woodLight: '#b98a5e',
  raw: '#e8786f', cooked: '#b47335', cash: '#54c06b', stone: '#aab6bd', water: '#78c2d3',
  orange: '#f08c3a', flame: '#ffb14e', gold: '#d9a24f', timber: '#b98a5e', timber: '#a5763f',
  roof: '#c05b4d', post: '#8a6540', smoke: '#e8edf1',
});
...
export class AssetFactory {
  ...
  geometry(kind) {
    ...
    if (!(kind in GEOMETRIES) || !(GEOMETRIES[kind] instanceof Object && GEOMETRIES[kind].constructor?.name?.includes('Geometry'))) {
      throw new RangeError(`Unknown geometry: ${kind}`);
    }
  ...
  createPlayer() {
    ...
    const leftArm = this.mesh('capsule', 'player', [0.32, 0.32, 0.32], [-0.42, 0.82, 0]);
    ...
    leftArm.geometry = this.geometry('capsule');
  ...
  createBear() {
    ...
    const legPositions = [[-0.5, -0.85], [0.5, -0.85], [-0.5, 0.85], [0.5, 0.85]];
    for (const [x, z] of legPositions) bear.add(this.mesh('box', 'bear', [0.32, 0.5, 0.32], [x, 0.25, z]));
    return bear;
  }

  createWood() {
    return this.#singleResource('lowCylinder', 'wood', [0.5, 0.24, 0.5], 0);
  }

  createRawMeat() {
    return this.#singleResource('box', 'raw', [0.46, 0.16, 0.34], 0);
  }

  createCookedMeat() {
    return this.#singleResource('box', 'cooked', [0.46, 0.18, 0.34], 0);
  }

  createCash() {
    const bundle = this.group('Cash');
    const base = this.mesh('box', 'cash', [0.42, 0.14, 0.3], [0, 0.07, 0]);
    const band = this.mesh('box', 'snow', [0.12, 0.16, 0.32], [0, 0.08, 0]);
    bundle.add(base, band);
    return bundle;
  }

  createFencePost() {
    const post = this.group('FencePost');
    const pole = this.mesh('box', 'post', [0.18, 1.05, 0.18], [0, 0.52, 0]);
    const cap = this.mesh('box', 'post', [0.3, 0.16, 0.3], [0, 1.1, 0]);
    const snow = this.mesh('box', 'snow', [0.34, 0.08, 0.34], [0, 1.2, 0]);
    post.add(pole, cap, snow);
    return post;
  }
  ...
  createCampfire() {
    ...
    for (let index = 0; index < 6; index++) {
      const stone = this.mesh('lowSphere', 'stone', [0.3, 0.22, 0.3], [Math.sin(index * Math.PI / 3) * 1.05, 0.1, Math.cos(index * Math.PI / 3) * 1.05]);
      fire.add(stone);
    }
    return fire;
  }
  ...
  createRock() {
    ...
    const body = this.mesh('lowSphere', 'stone', [0.9, 0.6, 0.8], [0, 0.25, 0]);
    const cap = this.mesh('lowSphere', 'snow', [0.72, 0.4, 0.64], [0, 0.5, 0]);
    rock.add(body, cap);
    return rock;
  }
  ...
  createMarket() {
    ...
    const counter = this.mesh('box', 'timber', [2.6, 1, 1], [0, 0.5, 0]);
    const counterTop = this.mesh('box', 'snow', [2.8, 0.12, 1.2], [0, 1.06, 0]);
    const postLeft = this.mesh('box', 'timber', [0.16, 2.2, 0.16], [-1.15, 1.1, -0.3]);
    const postRight = this.mesh('box', 'timber', [0.16, 2.2, 0.16], [1.15, 1.1, -0.3]);
    ...
    const crateA = this.mesh('box', 'wood', [0.5, 0.5, 0.5], [-1.9, 0.25, 0.3]);
    const crateB = this.mesh('box', 'wood', [0.4, 0.4, 0.4], [-1.75, 0.2, -0.35]);
    market.add(counter, counterTop, postLeft, postRight, roof, roofSnow, crateA, crateB);
    return market;
  }

  createResource(type) {
    const builders = { wood: () => this.createWood(), rawMeat: () => this.createRawMeat(), cookedMeat: () => this.createCookedMeat(), cash: () => this.createCash() };
    if (!(type in builders)) throw new RangeError(`Unknown resource type: ${type}`);
    return builders[type]();
  }

  #singleResource(kind, color, scale, rotation) {
    const item = this.group('Resource');
    const body = this.mesh(kind, color, scale, [0, scale[1] / 2, 0]);
...
const disc = (material) => material;
const vector = new Vector3();
const colorProbe = new Color();
void vector;
void colorProbe;
void disc;
